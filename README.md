...
11. `PYTHONPATH=. python scripts/export_dashboard_data.py && cd frontend && npm install && npm run dev`
   — the React showcase frontend (see "Frontend" below).

## Frontend

Two demo surfaces exist side by side, deliberately — neither replaces the other:

- **`app/dashboard.py`** (Streamlit) — the original, always-live demo surface.
  Queries the real pipeline on every page load.
- **`frontend/`** (React + TypeScript + Vite) — a polished showcase UI reading
  one precomputed JSON export (`frontend/public/data/dashboard.json`,
  generated by `scripts/export_dashboard_data.py`) instead of calling a live
  backend. That's a deliberate scope choice, not a shortcut hiding a weaker
  build: building a real API layer, a full SPA, *and* re-testing both
  end-to-end in the time available before the deadline was the riskier path
  for no real payoff, since every number in the export still comes from the
  exact same `agents.verdict.render_verdict()` / `agents.graph_builder
  .get_graph_features()` / `agents.red_team.evaluate_robustness()` calls the
  Streamlit version and the webhook receiver use — just computed once ahead
  of time instead of on every click. Full rationale in the project's
  `claude/frontend-react-plan.md` planning doc.

  Same 300-transaction held-out sample (seed 42) as the Streamlit dashboard,
  so the two surfaces never quietly disagree. Adds two views the Streamlit
  version doesn't have: an animated 5-agent architecture walkthrough, and a
  baseline-vs-hardened adversarial robustness comparison.

  Run: `PYTHONPATH=. python scripts/export_dashboard_data.py` (regenerate
  the data any time the model/graph changes, or after Neo4j has real data
  loaded via `scripts/load_graph_to_neo4j.py` — the export picks up live
  Neo4j automatically when it's reachable, local networkx fallback
  otherwise), then `cd frontend && npm install && npm run dev`.

  Explicit non-goals: no live scoring, no backend API, no auth. A showcase
  of real precomputed output, not a second production service.
...
- [x] Live Neo4j connectivity verified — `scripts/smoke_test_integrations.py`
      run from a real terminal confirms the AuraDB instance is reachable
      and authenticated (free-tier instances pause after inactivity; the
      script's own message says so if a fresh run fails). Fixed a real
      driver-cleanup bug found in the process: short-lived scripts weren't
      closing the Neo4j driver, so the interpreter tore its socket down
      mid-flight on exit and printed a misleading "Failed to write data to
      connection" warning that looked like a real failure but wasn't —
      see `agents/graph_builder_neo4j.py`'s `close_driver()`.
